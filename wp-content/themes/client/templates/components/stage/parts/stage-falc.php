...
<?php if ($title || $text || $image): ?>
    <section class="heroFalc">

        <article class="heroFalc__text-container" data-showup="true">
            <?php if ($title): ?>
                <h2 class="heroFalc__title">
                    <?= $title ?>
                </h2>
            <?php endif; ?>
            <?php if ($text): ?>
                <div class="heroFalc__text">
                    <?= $text ?>
                </div>
            <?php endif; ?>
        </article>
...
<?php endif; ?>
<section class="whoWeAreFalc">
    <div class="whoWeAreFalc__content">

        <article class="whoWeAreFalc__left" data-showup="true">
            <h2 class="whoWeAreFalc__title"><?= get_field('title_whoweare_falc') ?></h2>
            <div class="whoWeAreFalc__sub-text"><?= wp_kses_post(get_field('text_woweare_falc')) ?></div>
        </article>

        <article class="whoWeAreFalc__right" data-showup="true">
            <?php if (have_rows('plais_number_falc')): ?>

                <div class="whoWeAreFalc__Number">

                    <?php while (have_rows('plais_number_falc')) : the_row(); ?>

                        <article class="whoWeAreFalc__item" data-showup="true">
...
<section class="missionsFalc" data-showup="true">
        ...
        <?php if (have_rows('all_missions_falc')): ?>
            ...
            <?php while (have_rows('all_missions_falc')) : the_row(); ?>
                ...
                <article class="missionsFalc__item" data-showup="true">
                    ...
                    <h3 class="missionsFalc__item-title"><?= get_sub_field('missions_title_falc') ?></h3>
...
<section class="contactFpFalc">

    <article class="contactFpFalc__contenair" data-showup="true">
        <h2 class="contactFpFalc__title"><?= get_field('title_contact_falc') ?></h2>
        <div class="contactFpFalc__text"><?= wp_kses_post(get_field('text_whoscontacting_falc')) ?></div>
    </article>
    <?php if ($image): ?>
        <?= wp_get_attachment_image($image['id'], 'square-medium', false, [
                'alt' => $image['alt'],
